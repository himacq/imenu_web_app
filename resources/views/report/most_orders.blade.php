...
@section('content')
    <form id='form-data' action="{{ url('reports/most_orders') }}" method="post" >
    @csrf
    <!-- Main Content -->
                    ...
                    <div class="portlet light bordered">
                        <div class="portlet-title">
                            <div class="caption font-red-sunglo">
                                ...
                                <span class="caption-subject bold uppercase">{{trans('main.most_orders')}}</span>
                            </div>
                        ...
                        @if(!(empty($reportData)))
                            ...
                            @foreach($reportData as $data)
                                ...
                                <tr>
                                    ...
                                    @role('superadmin')
                                    <td>{{$data->product->category->restaurant->name}}</td>
                                    @endrole
                                    <td>{{$data->product->translate('name')}}</td>
                                    <td>{{$data->product->category->translate('name')}}</td>
                                    <td>
                                        @if($data->product->reviews->count())
                                        {{$data->product->reviews->sum('review_rank')/$data->product->reviews->count()}}
                                        @else
                                        0
                                        @endif
                                    </td>
                ...
                buttons: [
                    {
                        extend: 'print',
                        footer: true,
                        title: "{{trans('main.most_orders')}} {{trans('reports.from')}} {{$from}} {{trans('reports.to')}} {{$to}}"
                    },
                    { extend: 'excelHtml5', footer: true, title: "payments_{{$from}}_to_{{$to}}" },
                ],
